...
func resourceCloudCredentialsCreate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM error whilst creating Cloud API Credentials %s: %v", name, err)
	}
	d.SetId(name)
...
func resourceCloudCredentialsUpdate(d *schema.ResourceData, m interface{}) error {
	...
	err := client.Set("cloud_api_credentials", name, &cloudCredentialsConfiguration, nil)
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM error whilst updating Cloud API Credentials %s: %v", name, err)
	}
...
func resourceCloudCredentialsRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM error whilst retrieving Cloud API Credentials %s: %v", name, err)
	}
	...
	for _, key := range []string{
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Cloud Credentials error whilst setting attribute %s: %v", key, err)
		}
	}
