...
func resourceUserAuthenticatorSet(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM error whilst creating user authenticator %s: %v", name, err)
	}
	d.SetId(name)
...
func resourceUserAuthenticatorRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM error whilst retrieving user authenticator %s: %v", d.Id(), err)
	}
	...
	for _, attribute := range []string{"description", "enabled", "type"} {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM error whilst setting  attribute %s: %v", attribute, err)
		}
	...
	for _, sectionName := range []string{"ldap", "radius", "tacacs_plus"} {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM error whilst setting  attribute %s: %v", sectionName, err)
		}
	}
	return nil
}
