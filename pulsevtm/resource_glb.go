...
func buildListMaps(section []interface{}, attribute string) ([]map[string]interface{}, error) {
	...
	for _, item := range section {
		...
		if err != nil {
			return listMaps, fmt.Errorf("[ERROR] PulseVTM GLB error whilst building %s : %v", attribute, err)
		}
...
func resourceGLBSet(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM GLB error whilst creating %s: %v", name, err)
	}
	d.SetId(name)
...
func resourceGLBRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM GLB error whilst retrieving %s: %v", d.Id(), err)
	}
	...
	for _, attr := range []string{
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM GLB error whilst setting attribute %s: %v", attr, err)
		}
	}
	...
	for _, attribute := range []string{"dnssec_keys", "location_settings"} {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM GLB error whilst building %s : %v", attribute, err)
		}
		err = d.Set(attribute, listMaps)
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM GLB error whilst setting attribute %s: %v", attribute, err)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM GLB error whilst setting attribute log: %v", err)
	}
