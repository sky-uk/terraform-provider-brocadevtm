...
func resourceBandwidthCreate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Bandwidth error whilst creating %s: %v", name, err)
	}
	d.SetId(name)
...
func resourceBandwidthRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Bandwidth error whilst retrieving %s: %v", name, err)
	}
	...
	for _, key := range []string{"maximum", "note", "sharing"} {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Bandwidth error whilst setting attribute %s: %v", key, err)
		}
	}
	return nil
}
