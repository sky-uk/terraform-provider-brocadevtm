...
func resourceMonitorSet(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Monitor error whilst creating %s: %v", name, err)
	}
	d.SetId(name)
...
func resourceMonitorRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Monitor error whilst retrieving %s: %v", name, err)
	}
	...
	for _, key := range basicMonitorKeys() {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Monitor error whilst setting attribute %s: %v", key, err)
		}
	}
	...
	for _, sectionName := range []string{
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Monitor error whilst building section map to set: %v", err)
		}
		set = append(set, readSectionMap)
		err = d.Set(sectionName, set)
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Monitor error whilst setting section %s: %v", sectionName, err)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Monitor error whilst setting script_program: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Monitor error whilst setting script_arguments: %v", err)
	}
