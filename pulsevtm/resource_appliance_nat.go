...
func resourceApplianceNatCreate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Appliance/Nat error whilst creating: %s", err)
	}
...
func resourceApplianceNatRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Appliance/Nat error whilst retrieving: %s", err)
	}
	...
	for key := range resource.Schema {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Appliance/NAT error whilst setting attribute %s: %v", key, err)
		}
	}
	return nil
}
...
func resourceApplianceNatUpdate(d *schema.ResourceData, m interface{}) error {
	...
	err := client.Set("appliance/nat", "", natResource, nil)
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM ApplianceNat error whilst creating: %s", err)
	}
	return resourceApplianceNatRead(d, m)
}

// Actually you can't delete the resource on the Pulse server
...
func resourceApplianceNatDelete(d *schema.ResourceData, m interface{}) error {
	...
	err := client.Set("appliance/nat", "", natResource, nil)
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM ApplianceNat error whilst deleting all NAT rules: %s", err)
	}
	d.SetId("")
	return nil
}
