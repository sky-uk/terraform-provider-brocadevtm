...
func resourcePoolSet(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Pool error whilst creating/updating %s: %s", name, err)
	}
	d.SetId(name)
...
func resourcePoolRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("[ERROR] PulseVTM Pools error whilst retrieving %s: %v", d.Id(), err)
	}
	...
	for _, key := range basicPoolKeys() {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Pools error whilst setting attribute %s in state", key)
		}
	}
	...
	if _, ok := d.GetOk("nodes_list"); ok {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Pools error whilst setting attribute nodes_list in state")
		}
	}
	err = d.Set("nodes_table", poolsBasic["nodes_table"])
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Pools error whilst setting attribute nodes_table in state")
	}
	...
	for _, section := range []string{
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Pools error whilst setting attribute %s in state", section)
		}
	}
	return nil
}
