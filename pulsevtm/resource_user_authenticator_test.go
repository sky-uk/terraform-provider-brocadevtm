...
func TestAccPulseVTMUserAuthenticatorBasic(t *testing.T) {

	randomInt := acctest.RandInt()

	userAuthenticatorName := fmt.Sprintf("acctest_pulsevtm_user_authenticator-%d", randomInt)
	userAuthenticatorResourceName := "pulsevtm_user_authenticator.acctest"
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: func(state *terraform.State) error {
			return testAccPulseVTMUserAuthenticatorCheckDestroy(state, userAuthenticatorName)
		},
		Steps: []resource.TestStep{
			{
				Config:      testAccPulseUserAuthenticatorNoName(),
				ExpectError: regexp.MustCompile(`required field is not set`),
			},
			{
				Config:      testAccPulseUserAuthenticatorTooManyTacacs(),
				ExpectError: regexp.MustCompile(`tacacs_plus: attribute supports 1 item maximum, config has 2 declared`),
			},
			{
				Config:      testAccPulseUserAuthenticatorTooManyLDAP(),
				ExpectError: regexp.MustCompile(`ldap: attribute supports 1 item maximum, config has 2 declared`),
			},
			{
				Config:      testAccPulseUserAuthenticatorTooManyRadius(),
				ExpectError: regexp.MustCompile(`radius: attribute supports 1 item maximum, config has 2 declared`),
			},
			{

				Config: testAccPulseUserAuthenticatorCreate(userAuthenticatorName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMUserAuthenticatorExists(userAuthenticatorName, userAuthenticatorResourceName),
			...
			{
				Config: testAccPulseUserAuthenticatorUpdate(userAuthenticatorName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMUserAuthenticatorExists(userAuthenticatorName, userAuthenticatorResourceName),
					resource.TestCheckResourceAttr(userAuthenticatorResourceName, "name", userAuthenticatorName),
...
func testAccPulseVTMUserAuthenticatorCheckDestroy(state *terraform.State, name string) error {
	config := testAccProvider.Meta().(map[string]interface{})
	client := config["jsonClient"].(*api.Client)

	for _, rs := range state.RootModule().Resources {
		if rs.Type != "pulsevtm_user_authenticator" {
			continue
		}
		...
		for _, authenticator := range authenticators {
			if authenticator["name"] == name {
				return fmt.Errorf("[ERROR] Pulse vTM User Authenticator %s still exists", name)
			}
		}
	}
	return nil
...
func testAccPulseVTMUserAuthenticatorExists(name, resourceName string) resource.TestCheckFunc {
	return func(state *terraform.State) error {
		rs, ok := state.RootModule().Resources[resourceName]
		if !ok {
			return fmt.Errorf("\n[ERROR] Pulse vTM User Authenticator %s wasn't found in resources", name)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("\n[ERROR] Pulse vTM User Authenticator ID not set for %s in resources", name)
		}
		config := testAccProvider.Meta().(map[string]interface{})
		client := config["jsonClient"].(*api.Client)
		authenticators, err := client.GetAllResources("user_authenticators")
		...
		return fmt.Errorf("[ERROR] Pulse vTM User Authenticator %s not found on remote vTM", name)
	}
}

func testAccPulseUserAuthenticatorCreate(name string) string {
	return fmt.Sprintf(`
...
func testAccPulseUserAuthenticatorUpdate(name string) string {
	return fmt.Sprintf(`
       resource "pulsevtm_user_authenticator" "acctest" {
       name = "%s"
...
func testAccPulseUserAuthenticatorNoName() string {
	return fmt.Sprintf(`
       resource "pulsevtm_user_authenticator" "acctest" {
...
func testAccPulseUserAuthenticatorTooManyTacacs() string {
	return fmt.Sprintf(`
       resource "pulsevtm_user_authenticator" "acctest" {
       	       name = "invalidTypeUA"
...
func testAccPulseUserAuthenticatorTooManyLDAP() string {
	return fmt.Sprintf(`
       resource "pulsevtm_user_authenticator" "acctest" {
       	       name = "invalidTypeUA"
	       description = "Invalid Type Acceptance Test"
...
func testAccPulseUserAuthenticatorTooManyRadius() string {
	return fmt.Sprintf(`
       resource "pulsevtm_user_authenticator" "acctest" {
       	       name = "invalidTypeUA"
	       description = "Invalid Type Acceptance Test"
