package pulsevtm

import (
	"fmt"
	"github.com/hashicorp/terraform/helper/acctest"
	"github.com/hashicorp/terraform/helper/resource"
	"github.com/hashicorp/terraform/terraform"
	"github.com/sky-uk/go-pulse-vtm/api"
	"regexp"
	"testing"
)

func TestAccPulseVTMSSLCasBasic(t *testing.T) {

	randomInt := acctest.RandInt()
	sslCasConfigName := fmt.Sprintf("acctest_pulsevtm_ssl_cas_file-%d", randomInt)
	sslCasConfigResourceName := "pulsevtm_ssl_cas_file.acctest"
	sslServerKeyName := fmt.Sprintf("acctest_pulsevtm_ssl_cas_file-%d", randomInt)

	fmt.Printf("\n\nSSL cas config file is %s.\n\n", sslCasConfigName)

	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
		Providers: testAccProviders,
		CheckDestroy: func(state *terraform.State) error {
			return testAccPulseVTMSSLCasConfigCheckDestroy(state, sslCasConfigName)
		},
		Steps: []resource.TestStep{
			{
				Config:      testAccPulseVTMSSLCasConfigNoNameTemplate(),
				ExpectError: regexp.MustCompile(`required field is not set`),
			},
			{
				Config: testAccPulseSSLCasConfigCreateTemplate(sslCasConfigName, sslServerKeyName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMSSLCasConfigExists(sslCasConfigName, sslCasConfigResourceName),
					resource.TestCheckResourceAttr(sslCasConfigResourceName, "name", sslCasConfigName),
					resource.TestCheckResourceAttr(sslCasConfigResourceName, "ssl_cas_config", "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"),
				),
			},
			{
				Config: testAccPulseSSLCasConfigUpdateTemplate(sslCasConfigName, sslServerKeyName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMSSLCasConfigExists(sslCasConfigName, sslCasConfigResourceName),
					resource.TestCheckResourceAttr(sslCasConfigResourceName, "name", sslCasConfigName),
					resource.TestCheckResourceAttr(sslCasConfigResourceName, "ssl_cas_config", "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN X509 CRL-----\\n-----END X509 CRL-----\n"),
				),
			},
		},
	})
}

func testAccPulseVTMSSLCasConfigCheckDestroy(state *terraform.State, name string) error {

	config := testAccProvider.Meta().(map[string]interface{})
	client := config["jsonClient"].(*api.Client)

	client.WorkWithConfigurationResources()

	for _, rs := range state.RootModule().Resources {

		if rs.Type != "pulsevtm_ssl_cas_file" {
			continue
		}
		if id, ok := rs.Primary.Attributes["id"]; ok && id != "" {
			return nil
		}
		sslCasList, err := client.GetAllResources("ssl/cas")
		if err != nil {
			return fmt.Errorf("[ERROR] Pulse vTM error occurred while retrieving list of SSL cas configs: %v", err)
		}
		for _, childSSLCasConfig := range sslCasList {
			if childSSLCasConfig["name"] == name {
				return fmt.Errorf("[ERROR] Pulse vTM SSL cas config %s still exists", name)
			}
		}
	}
	return nil
}

func testAccPulseVTMSSLCasConfigExists(name, resourceName string) resource.TestCheckFunc {
	return func(state *terraform.State) error {

		config := testAccProvider.Meta().(map[string]interface{})
		client := config["jsonClient"].(*api.Client)

		client.WorkWithConfigurationResources()

		rs, ok := state.RootModule().Resources[resourceName]
		if !ok {
			return fmt.Errorf("\n[ERROR] Pulse vTM SSL cas config %s wasn't found in resources", name)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("\n[ERROR] Pulse vTM SSL cas config ID not set for %s in resources", name)
		}

		sslCasConfigList, err := client.GetAllResources("ssl/cas")
		if err != nil {
			return fmt.Errorf("[ERROR] Pulse vTM error occurred while retrieving list of SSL cas configs: %v", err)
		}
		for _, sslCasConfig := range sslCasConfigList {
			if sslCasConfig["name"] == name {
				return nil
			}
		}
		return fmt.Errorf("[ERROR] Pulse vTM SSL cas config %s not found on remote vTM", name)
	}
}

func testAccPulseVTMSSLCasConfigNoNameTemplate() string {
	return fmt.Sprintf(`
resource "pulsevtm_ssl_cas_file" "acctest" {
}
`)
}

func testAccPulseVTMSSLCasConfigPrepare(sslServerKeyName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_ssl_server_key" "acctest" {
  name = "%s"
  private = <<PRIVATE
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
PRIVATE
  public = <<PUBLIC
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
PUBLIC
  request = <<REQUEST
-----BEGIN CERTIFICATE REQUEST-----

REQUEST
}
`, sslServerKeyName)
}

func testAccPulseSSLCasConfigCreateTemplate(sslCasConfigName, sslServerKeyName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_ssl_cas_file" "acctest" {
  name = "%s"
  ssl_cas_config = <<SSL_CAS_CONFIG
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
SSL_CAS_CONFIG
  depends_on = ["pulsevtm_ssl_server_key.acctest"]
}
%s
`, sslCasConfigName, testAccPulseVTMSSLCasConfigPrepare(sslServerKeyName))
}

func testAccPulseSSLCasConfigUpdateTemplate(sslCasConfigName, sslServerKeyName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_ssl_cas_file" "acctest" {
  name = "%s"
  ssl_cas_config = <<SSL_CAS_CONFIG
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN X509 CRL-----

-----END X509 CRL-----
SSL_CAS_CONFIG
  depends_on = ["pulsevtm_ssl_server_key.acctest"]
}
%s
`, sslCasConfigName, testAccPulseVTMSSLCasConfigPrepare(sslServerKeyName))
}
