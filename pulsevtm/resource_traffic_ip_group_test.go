...
func TestAccPulseVTMTrafficIpGroupBasic(t *testing.T) {

	randomInt := acctest.RandInt()

	trafficIPGroupName := fmt.Sprintf("acctest_pulsevtm_traffic_ip_group-%d", randomInt)
	trafficIPGroupResourceName := "pulsevtm_traffic_ip_group.acctest"
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: func(state *terraform.State) error {
			return testAccPulseVTMTrafficIPGroupCheckDestroy(state, trafficIPGroupName)
		},
		Steps: []resource.TestStep{
			{
				Config:      testAccPulseVTMTrafficIPGroupNoName(),
				ExpectError: regexp.MustCompile(`required field is not set`),
			},
			{
				Config:      testAccPulseVTMTrafficIPGroupInvalidBackendTrafficIPs(trafficIPGroupName),
				ExpectError: regexp.MustCompile(`backend_traffic_ips: should be a list`),
			},
			{
				Config:      testAccPulseVTMTrafficIPGroupInvalidIPAssignmentMode(trafficIPGroupName),
				ExpectError: regexp.MustCompile(`expected ip_assignment_mode to be one of \[alphabetic balanced\], got SOME_INVALID_MODE`),
			},
			{
				Config:      testAccPulseVTMTrafficIPGroupInvalidMode(trafficIPGroupName),
			...
			{
				Config:      testAccPulseVTMTrafficIPGroupInvalidIPAddress(trafficIPGroupName),
				ExpectError: regexp.MustCompile(`ipaddresses: should be a list`),
			},
			{
				Config:      testAccPulseVTMTrafficIPGroupInvalidMulticastIP(trafficIPGroupName),
				ExpectError: regexp.MustCompile(`must be a valid multicast IP \(224.0.0.0 - 239.255.255.255\)`),
			},
			{
				Config:      testAccPulseVTMTrafficIPGroupInvalidUnsignedInt(trafficIPGroupName),
				ExpectError: regexp.MustCompile(`can't be negative`),
			},
			{
				Config:      testAccPulseVTMTrafficIPGroupInvalidRHIProtocol(trafficIPGroupName),
				ExpectError: regexp.MustCompile(`expected rhi_protocols to be one of \[ospf bgp\], got INVALID_PROTOCOL`),
			},
			{
				Config: testAccPulseVTMTrafficIPGroupCreateTemplate(trafficIPGroupName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMTrafficIPGroupExists(trafficIPGroupName, trafficIPGroupResourceName),
			...
			{
				Config: testAccPulseVTMTrafficIPGroupUpdateTemplate(trafficIPGroupName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMTrafficIPGroupExists(trafficIPGroupName, trafficIPGroupResourceName),
					resource.TestCheckResourceAttr(trafficIPGroupResourceName, "name", trafficIPGroupName),
			...
			{
				Config: testAccPulseVTMTrafficIPGroupUpdate2Template(trafficIPGroupName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMTrafficIPGroupExists(trafficIPGroupName, trafficIPGroupResourceName),
					resource.TestCheckResourceAttr(trafficIPGroupResourceName, "name", trafficIPGroupName),
					resource.TestCheckResourceAttr(trafficIPGroupResourceName, "backend_traffic_ips.#", "1"),
...
func testAccPulseVTMTrafficIPGroupCheckDestroy(state *terraform.State, name string) error {

	config := testAccProvider.Meta().(map[string]interface{})
	client := config["jsonClient"].(*api.Client)

	for _, rs := range state.RootModule().Resources {
		if rs.Type != "pulsevtm_traffic_ip_group" {
			continue
		}
		...
		if err != nil {
			return fmt.Errorf("[ERROR] Pulse vTM traffic IP group error retrieving the list of traffic IP groups")
		}
		for _, trafficIPGroupItem := range trafficIPGroups {
			if trafficIPGroupItem["name"] == name {
				return fmt.Errorf("[ERROR] Pulse vTM traffic IP group %s still exists", name)
			}
		}
	}
	return nil
...
func testAccPulseVTMTrafficIPGroupExists(trafficIPGroupName, trafficIPGroupResourceName string) resource.TestCheckFunc {
	return func(state *terraform.State) error {

		rs, ok := state.RootModule().Resources[trafficIPGroupResourceName]
		if !ok {
			return fmt.Errorf("\n[ERROR] Pulse vTM Traffic IP Group resource %s not found in resources", trafficIPGroupName)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("\n[ERROR] Pulse vTM Traffic IP Group ID not set in resources")
		}
		...
		return fmt.Errorf("[ERROR] Pulse vTM Traffic IP Group %s not found on remote vTM", trafficIPGroupName)
	}
}

func testAccPulseVTMTrafficIPGroupNoName() string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
}
`)
}

func testAccPulseVTMTrafficIPGroupInvalidBackendTrafficIPs(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
...
func testAccPulseVTMTrafficIPGroupInvalidIPAssignmentMode(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
...
func testAccPulseVTMTrafficIPGroupInvalidMode(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
...
func testAccPulseVTMTrafficIPGroupInvalidIPAddress(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
...
func testAccPulseVTMTrafficIPGroupInvalidMulticastIP(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
...
func testAccPulseVTMTrafficIPGroupInvalidUnsignedInt(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
...
func testAccPulseVTMTrafficIPGroupInvalidRHIProtocol(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
...
func testAccPulseVTMTrafficIPGroupCreateTemplate(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
  backend_traffic_ips=["192.168.10.49"]
...
func testAccPulseVTMTrafficIPGroupUpdateTemplate(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
...
func testAccPulseVTMTrafficIPGroupUpdate2Template(trafficIPGroupName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_traffic_ip_group" "acctest" {
  name = "%s"
