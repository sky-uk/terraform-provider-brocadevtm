...
func resourceDNSZoneCreate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM DNS zone error whilst creating %s: %v", name, err)
	}
...
func resourceDNSZoneRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		d.SetId("")
		return fmt.Errorf("[ERROR] PulseVTM DNS zone error whilst reading %s: %v", name, err)
	}
	d.SetId(name)
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM DNS zone error whilst setting attribute origin: %v", err)
	}
	err = d.Set("zone_file", basic["zonefile"])
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM DNS zone error whilst setting attribute zone_file: %v", err)
	}
...
func resourceDNSZoneUpdate(d *schema.ResourceData, m interface{}) error {
	...
	err := client.Set("dns_server/zones", name, &res, nil)
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM DNS zone error whilst updating %s: %v", name, err)
	}
