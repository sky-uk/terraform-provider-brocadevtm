...
func TestAccPulseVTMVirtualServerBasic(t *testing.T) {

	randomInt := acctest.RandInt()

	virtualServerName := fmt.Sprintf("acctest_pulsevtm_virtual_server-%d", randomInt)
	resourceName := "pulsevtm_virtual_server.acctest"
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: func(state *terraform.State) error {
			return testAccPulseVTMVirtualServerCheckDestroy(state, virtualServerName)
		},
		Steps: []resource.TestStep{
			{
				Config:      testAccPulseVTMVirtualServerNoName(),
				ExpectError: regexp.MustCompile(`required field is not set`),
			},
			{
				Config: testAccPulseVTMVirtualServerCreate(virtualServerName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMVirtualServerExists(virtualServerName, resourceName),
			...
			{
				Config: testAccPulseVTMVirtualServerUpdate(virtualServerName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMVirtualServerExists(virtualServerName, resourceName),
					resource.TestCheckResourceAttr(resourceName, "name", virtualServerName),
...
func testAccPulseVTMVirtualServerCheckDestroy(state *terraform.State, name string) error {
	config := testAccProvider.Meta().(map[string]interface{})
	client := config["jsonClient"].(*api.Client)

	for _, rs := range state.RootModule().Resources {
		if rs.Type != "pulsevtm_virtual_server" {
			continue
		}
		...
		if client.StatusCode == http.StatusOK {
			return fmt.Errorf("[ERROR] Pulse vTM Check Destroy Error: Virtual Server %s still exists", name)
		}
		if client.StatusCode == http.StatusNotFound {
			return nil
		}

		return fmt.Errorf("[ERROR] Pulse vTM error occurred while retrieving Virtual Server: %s", err)

	}
	return nil
}

func testAccPulseVTMVirtualServerExists(name, resourceName string) resource.TestCheckFunc {
	return func(state *terraform.State) error {
		rs, ok := state.RootModule().Resources[resourceName]
		if !ok {
			return fmt.Errorf("\n[ERROR] Pulse vTM Virtual Server %s wasn't found in resources", name)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("\n[ERROR] Pulse vTM Virtual Server ID not set for %s in resources", name)
		}
		...
		err := client.GetByName("virtual_servers", name, &vs)
		if err != nil {
			return fmt.Errorf("[ERROR] Pulse vTM Virtual Server - error while retrieving virtual server %s: %s", name, err)
		}
		if client.StatusCode == http.StatusOK {
			return nil
		}
		return fmt.Errorf("[ERROR] Pulse vTM Virtual Server %s not found on remote vTM", name)
	}
}

func testAccPulseVTMVirtualServerNoName() string {
	return `
resource "pulsevtm_virtual_server" "acctest" {
pool = "test-pool"
port = 80
}
`
...
func testAccPulseVTMVirtualServerCreate(virtualServerName string) string {
	return fmt.Sprintf(`
...
func testAccPulseVTMVirtualServerUpdate(virtualServerName string) string {
	return fmt.Sprintf(`
