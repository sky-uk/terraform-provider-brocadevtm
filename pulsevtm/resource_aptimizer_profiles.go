...
func resourceAptimizerProfileCreate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM error whilst creating Aptimizer Profile %s: %v", name, err)
	}
	d.SetId(name)
...
func resourceAptimizerProfileRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM error whilst retrieving Aptimizer Profile %s: %v", name, err)
	}
	...
	for _, key := range []string{"background_after", "background_on_additional_resources", "mode", "show_info_bar"} {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM error whilst setting attribute %s: %v", key, err)
		}
	}
...
func resourceAptimizerProfileUpdate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM error whilst updating Aptimizer Profile %s: %v", d.Id(), err)
	}
