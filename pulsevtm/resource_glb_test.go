...
func TestAccPulseVTMGLBBasic(t *testing.T) {

	randomInt := acctest.RandInt()
	glbName := fmt.Sprintf("acctest_pulsevtm_glb-%d", randomInt)
	glbResourceName := "pulsevtm_glb.acctest"
	fmt.Printf("\n\nGLB is %s.\n\n", glbName)
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: func(state *terraform.State) error {
			return testAccPulseVTMGLBCheckDestroy(state, glbName)
		},
		Steps: []resource.TestStep{
			{
				Config:      testAccPulseVTMGLBNoNameTemplate(),
				ExpectError: regexp.MustCompile(`required field is not set`),
			},
			{
				Config: testAccPulseGLBCreateTemplate(glbName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMGLBExists(glbName, glbResourceName),
			...
			{
				Config: testAccPulseGLBUpdateTemplate(glbName),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMGLBExists(glbName, glbResourceName),
					resource.TestCheckResourceAttr(glbResourceName, "name", glbName),
...
func testAccPulseVTMGLBCheckDestroy(state *terraform.State, glbName string) error {
	config := testAccProvider.Meta().(map[string]interface{})
	client := config["jsonClient"].(*api.Client)

	for _, rs := range state.RootModule().Resources {
		if rs.Type != "pulsevtm_glb" {
			continue
		}
		...
		if err != nil {
			return fmt.Errorf("[ERROR] Pulse vTM GLB - error while retrieving GLB: %v", err)
		}
		for _, glb := range glbServices {
			if glb["name"] == glbName {
				return fmt.Errorf("[ERROR] Pulse vTM GLB %s still exists", glbName)
			}
		}
	}
	return nil
...
func testAccPulseVTMGLBExists(glbName, glbResourceName string) resource.TestCheckFunc {
	return func(state *terraform.State) error {
		rs, ok := state.RootModule().Resources[glbResourceName]
		if !ok {
			return fmt.Errorf("\n[ERROR] Pulse vTM GLB %s wasn't found in resources", glbName)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("\n[ERROR] Pulse vTM GLB ID not set for %s in resources", glbName)
		}
		config := testAccProvider.Meta().(map[string]interface{})
		client := config["jsonClient"].(*api.Client)
		glbServices, err := client.GetAllResources("glb_services")
		...
		return fmt.Errorf("[ERROR] Pulse vTM  GLB %s not found on remote vTM", glbName)
	}
}

func testAccPulseVTMGLBNoNameTemplate() string {
	return `
resource "pulsevtm_glb" "acctest" {

}
`
}

func testAccPulseGLBCreateTemplate(glbName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_glb" "acctest" {
  name = "%s"
  ...
  log = {
	  enabled = true
	  filename = "/var/log/pulsevtm/test.log"
  }
}
`, glbName)
}

func testAccPulseGLBUpdateTemplate(glbName string) string {
	return fmt.Sprintf(`
resource "pulsevtm_glb" "acctest" {
  name = "%s"
  algorithm = "geo"
