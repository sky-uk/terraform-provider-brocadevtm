...
func TestAccPulseVTMApplianceNat(t *testing.T) {

	randomInt := acctest.RandInt()
	name := fmt.Sprintf("acctest_pulsevtm_appliance_nat-%d", randomInt)
	resourceName := fmt.Sprintf("pulsevtm_appliance_nat.acctest")
	fmt.Printf("\n\nAppliance Nat is %s.\n\n", name)
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: func(state *terraform.State) error {
			return testAccPulseVTMApplianceNatCheckDestroy(state, name)
		},
		Steps: []resource.TestStep{
			{
				Config: testAccPulseVTMApplianceNatEmptyResource(),
				Check: resource.ComposeTestCheckFunc(
					testAccPulseVTMApplianceNatExists(name, resourceName),
				),
			},
			{
...
func testAccPulseVTMApplianceNatCheckDestroy(state *terraform.State, name string) error {

	for _, rs := range state.RootModule().Resources {
		if rs.Type != "pulsevtm_appliance_nat" {
			continue
		}
		...
		if err != nil {
			return fmt.Errorf("[ERROR] Pulse vTM error whilst retrieving appliance nat: %+v", err)
		}
		for _, resource := range resources {
			if resource["name"] == name {
				return fmt.Errorf("[ERROR] Pulse vTM Appliance nat %s still exists", name)
			}
		}
	}
	return nil
...
func testAccPulseVTMApplianceNatExists(name, resourceName string) resource.TestCheckFunc {
	return func(state *terraform.State) error {

		rs, ok := state.RootModule().Resources[resourceName]
		if !ok {
			return fmt.Errorf("\n[ERROR] Pulse vTM Appliance Nat %s wasn't found in resources", name)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("\n[ERROR] Pulse vTM Appliance Nat ID not set for %s in resources", name)
		}
		config := testAccProvider.Meta().(map[string]interface{})
		client := config["jsonClient"].(*api.Client)
		client.WorkWithConfigurationResources()
		...
		if err != nil {
			return fmt.Errorf("[ERROR] Pulse vTM error whilst retriving appliance nat: %v", err)
		}
		return nil
	}
}

func testAccPulseVTMApplianceNatEmptyResource() string {
	return fmt.Sprintf(`
resource "pulsevtm_appliance_nat" "acctest" {
...
resource "pulsevtm_appliance_nat" "acctest" {
  depends_on = ["pulsevtm_traffic_ip_group.acctest", "pulsevtm_pool.acctest"]
  many_to_one_all_ports = [{
	  rule_number = 4765348
	  pool = "%s"
	  tip = "%s"
...
resource "pulsevtm_appliance_nat" "acctest" {
  depends_on = [ "pulsevtm_traffic_ip_group.acctest", "pulsevtm_pool.acctest", "pulsevtm_virtual_server.acctest" ]
  many_to_one_all_ports = []
  many_to_one_port_locked = [{
	  rule_number = 20001
	  pool = "%s"
