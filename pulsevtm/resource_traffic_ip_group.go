...
func getTrafficManagers(m interface{}) ([]string, error) {
	...
	if err != nil {
		return trafficManagers, fmt.Errorf("[ERROR] PulseVTM Traffic Managers error whilst retrieving the list of Traffic Managers: %v", err)
	}
...
func resourceTrafficIPGroupSet(d *schema.ResourceData, m interface{}) error {
	...
	if len(trafficIPGroupProperties["basic"].(map[string]interface{})["machines"].([]interface{})) == 0 {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Traffic IP Group error whilst creating %s: %v", name, err)
		}
	...
	if err != nil {
		return fmt.Errorf("[ERROR] PulseVTM Traffic IP Group error whilst creating/updating %s: %s", name, err)
	}
	d.SetId(name)
...
func resourceTrafficIPGroupRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("[ERROR] PulseVTM Traffic IP Group error whilst retrieving %s: %v", name, err)
	}
	...
	for _, key := range basicTrafficIPGroupKeys() {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] PulseVTM Traffic IP Group error whilst setting attribute %s in state", key)
		}
	}
	return nil
}
